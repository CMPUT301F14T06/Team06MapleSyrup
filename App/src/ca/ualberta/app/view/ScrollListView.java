...
public class ScrollListView extends ListView implements OnScrollListener {
	...
	// -- header view
	private ScrollListViewHeader mHeaderView;
	...
	// -- footer view
	private ScrollListViewFooter mFooterView;
	...
	private void initWithContext(Context context) {
		...
		// init header view
		mHeaderView = new ScrollListViewHeader(context);
		mHeaderViewContent = (RelativeLayout) mHeaderView
				.findViewById(R.id.scrolllistview_header_content);
		mHeaderTimeView = (TextView) mHeaderView
				.findViewById(R.id.scrolllistview_header_time);
		addHeaderView(mHeaderView);

		// init footer view
		mFooterView = new ScrollListViewFooter(context);
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();
			mFooterView.setState(ScrollListViewFooter.STATE_NORMAL);
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(ScrollListViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(ScrollListViewHeader.STATE_NORMAL);
			}
		}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setState(ScrollListViewFooter.STATE_READY);
			} else {
				mFooterView.setState(ScrollListViewFooter.STATE_NORMAL);
			}
		}
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(ScrollListViewFooter.STATE_LOADING);
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullRefresh
					...
					mHeaderView.setState(ScrollListViewHeader.STATE_REFRESHING);
					if (mListViewListener != null) {
	...
	public void setScrollListViewListener(IXListViewListener l) {
		mListViewListener = l;
	}
